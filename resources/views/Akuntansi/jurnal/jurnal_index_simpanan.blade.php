	...
	<div class="col-md-12">
		<section class="panel no-b">
			<div class="panel-body">
				<div class="row">
					<form class="form-horizontal" role="form" method="get" action="{!! url('akuntansi/jurnal/search/simpanan') !!}">
						<div class="col-md-6">
					...
					<div class="col-md-12">
						<div class="box-tab">
							<ul class="nav nav-tabs">
								...
								<li class=""><a href="{{url('akuntansi/jurnal/kas')}}">Kas</a></li>
								...
								<li class=""><a href="{{url('akuntansi/jurnal/search/kas?kata_kunci='.$kata_kunci.'&'.'akun_perkiraan='.$akun_perkiraan.'&'.'posting='.$posting.'&'.'datefrom='.$date1.'&'.'dateto='.$date2)}}">Kas</a></li>
							...
							<div class="tab-content">
								<div class="tab-pane fade active in" id="simpanan">
									<div class="row">
										<div class="col-md-6">
											<a target="_blank" href="{{url('akuntansi/jurnal/cetak/Simpanan/'.$status.'?kata_kunci='.$kata_kunci.'&'.'akun_perkiraan='.$akun_perkiraan.'&'.'posting='.$posting.'&'.'datefrom='.$date1.'&'.'dateto='.$date2)}}" class="btn btn-sm btn-info mb5"><i class="ti-printer mr5"></i>Cetak</a>
										</div>
										<div class="col-md-6">
											<div class="pull-right">
												Total data dalam semua transaksi : {!! $JurnalHeaderSimpananjml !!}
											</div>
										</div>
									</div>
									<div class="row">
						                        ...
						                        <table class="table table-bordered table-striped mg-t">
						                            <thead>
                                                        <tr class="bg-color">
                                                            ...
                                                            <th><input type="checkbox" name="checkAll" id="TableSimpanan"></th>
							                            </tr>
						                            </thead>
						                            <tbody>
                                                        @if($JurnalHeaderSimpananjml <= 0)
    						                            	<tr>
    						                            		<td colspan="10"><center>Data Tidak Ada</center></td>
    						                            	</tr>
						                            	@else
        						                            <?php
                                                                $i = ($JurnalHeaderSimpanan->currentPage() - 1) * $JurnalHeaderSimpanan->perPage() + 1;
                                                                $prev_kode_jurnalSimpanan = '';
                                                            ?>
        						                            @foreach($JurnalHeaderSimpanan as $row)
        						                            	<?php
                                                                    if($akun_perkiraan=="semua" or $akun_perkiraan ==""){
                                                                        $JurnalDetailSimpanan = App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->get();
                                                                        $total_detailSimpanan = App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->count();
                                                                    } else {
                                                                        $JurnalDetailSimpanan = App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->where('id_akun', $akun_perkiraan)->get();
                                                                        $total_detailSimpanan= App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->where('id_akun', $akun_perkiraan)->count();
                                                                    }

        						                            	?>
        						                            	@foreach($JurnalDetailSimpanan as $datajurnal)
            							                            <tr>
                                                                        <?php
                                                                            if($row['kode_jurnal'] != $prev_kode_jurnalSimpanan){
                                                                                echo '<td class="text-center" rowspan="'.$total_detailSimpanan.'">'.$i++.'.</td>';
                                                                                echo '<td rowspan="'.$total_detailSimpanan.'">'.$row['kode_jurnal'].'</td>';
                                                                            }
                                                                        ?>
                                                                        ...
                                                                        <?php
                                                                            if($row['kode_jurnal'] != $prev_kode_jurnalSimpanan){
                                                                                if($datajurnal->posting==1){
                                                                                    echo '<td rowspan="'.$total_detailSimpanan.'"><input type="checkbox" disabled placeholder="" name="cbpilih['.$row->id.']"/></td>';
                                                                                } else {
                                                                                    echo '<td rowspan="'.$total_detailSimpanan.'"><input type="checkbox" placeholder="" name="cbpilih['.$row->id.']"/></td>';
                                                                                }
                                                                            }
                                                                        ?>
                                                                        <?php $prev_kode_jurnalSimpanan = $row['kode_jurnal']; ?>
            							                            </tr>
        							                            @endforeach
        							                        @endforeach
                                                        @endif
                                                ...
                                                <div class="row">
                                                        <div class="col-md-11">
                                                            <div class="form-group pull-right">
                                                                @if($status=="index")
                                                                {{ $JurnalHeaderSimpanan->links() }}
                                                                @elseif($status=="cari")
                                                                {!! str_replace('?','?kata_kunci='.$kata_kunci.'&'.'akun_perkiraan='.$akun_perkiraan.'&'.'posting='.$posting.'&'.'datefrom='.$date1.'&'.'dateto='.$date2.'&',$JurnalHeaderSimpanan->links()) !!}
                                                                @endif
                            								</div>
                                                        </div>
                                						<div class="col-md-1">
...
<script type="text/javascript">
    ...
    $('#TableSimpanan').click(function (e) {
