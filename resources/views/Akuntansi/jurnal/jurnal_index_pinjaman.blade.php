	...
	<div class="col-md-12">
		<section class="panel no-b">
			<div class="panel-body">
				<div class="row">
					<form class="form-horizontal" role="form" method="get" action="{!! url('akuntansi/jurnal/search/pinjaman') !!}">
						<div class="col-md-6">
						...
						<div class="box-tab">
							...
							<div class="tab-content">
								<div class="tab-pane fade active in" id="pinjaman">
									<div class="row">
										<div class="col-md-6">
											<a target="_blank" href="{{url('akuntansi/jurnal/cetak/Pinjaman/'.$status.'?kata_kunci='.$kata_kunci.'&'.'akun_perkiraan='.$akun_perkiraan.'&'.'posting='.$posting.'&'.'datefrom='.$date1.'&'.'dateto='.$date2)}}" class="btn btn-sm btn-info mb5"><i class="ti-printer mr5"></i>Cetak</a>
										</div>
										<div class="col-md-6">
											<div class="pull-right">
												Total data dalam semua transaksi : {!! $JurnalHeaderPinjamanjml !!}
											</div>
										</div>
									</div>
									<div class="row">
						                        ...
						                        <table class="table table-bordered table-striped mg-t">
						                            <thead>
                                                        <tr class="bg-color">
                                                            ...
                                                            <th><input type="checkbox" name="checkAll" id="TablePinjaman"></th>
							                            </tr>
						                            </thead>
						                            <tbody>
                                                        @if($JurnalHeaderPinjamanjml <= 0)
    						                            	<tr>
    						                            		<td colspan="10"><center>Data Tidak Ada</center></td>
    						                            	</tr>
						                            	@else
        						                            <?php
                                                                $i = ($JurnalHeaderPinjaman->currentPage() - 1) * $JurnalHeaderPinjaman->perPage() + 1;
                                                                $prev_kode_jurnalPinjaman = '';
                                                            ?>
        						                            @foreach($JurnalHeaderPinjaman as $row)
        						                            	<?php
                                                                    if($akun_perkiraan=="semua" or $akun_perkiraan ==""){
                                                                        $JurnalDetailPinjaman = App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->get();
                                                                        $total_detailPinjaman = App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->count();
                                                                    } else {
                                                                        $JurnalDetailPinjaman = App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->where('id_akun', $akun_perkiraan)->get();
                                                                        $total_detailPinjaman= App\Model\Akuntansi\JurnalDetail::where('id_header', $row->id)->where('id_akun', $akun_perkiraan)->count();
                                                                    }

        						                            	?>
        						                            	@foreach($JurnalDetailPinjaman as $datajurnal)
            							                            <tr>
                                                                        <?php
                                                                            if($row['kode_jurnal'] != $prev_kode_jurnalPinjaman){
                                                                                echo '<td class="text-center" rowspan="'.$total_detailPinjaman.'">'.$i++.'.</td>';
                                                                                echo '<td rowspan="'.$total_detailPinjaman.'">'.$row['kode_jurnal'].'</td>';
                                                                            }
                                                                        ?>
                                                                        ...
                                                                        <?php
                                                                            if($row['kode_jurnal'] != $prev_kode_jurnalPinjaman){
                                                                                if($datajurnal->posting==1){
                                                                                    echo '<td rowspan="'.$total_detailPinjaman.'"><input type="checkbox" disabled placeholder="" name="cbpilih['.$row->id.']"/></td>';
                                                                                } else {
                                                                                    echo '<td rowspan="'.$total_detailPinjaman.'"><input type="checkbox" placeholder="" name="cbpilih['.$row->id.']"/></td>';
                                                                                }
                                                                            }
                                                                        ?>
                                                                        <?php $prev_kode_jurnalPinjaman = $row['kode_jurnal']; ?>
            							                            </tr>
        							                            @endforeach
        							                        @endforeach
                                                        @endif
                                                ...
                                                <div class="row">
                                                        <div class="col-md-11">
                                                            <div class="form-group pull-right">
                                                                @if($status=="index")
                                                                {{ $JurnalHeaderPinjaman->links() }}
                                                                @elseif($status=="cari")
                                                                {!! str_replace('?','?kata_kunci='.$kata_kunci.'&'.'akun_perkiraan='.$akun_perkiraan.'&'.'posting='.$posting.'&'.'datefrom='.$date1.'&'.'dateto='.$date2.'&',$JurnalHeaderPinjaman->links()) !!}
                                                                @endif
                            								</div>
                                                        </div>
                                						<div class="col-md-1">
...
<script type="text/javascript">
    ...
    $('#TablePinjaman').click(function (e) {
