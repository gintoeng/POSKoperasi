...
<table width="100%">
    ...
    @foreach($simpanan as $key => $value)
        <tr>
            ...
            <td>{!! $value['nama'] !!}</td>
                @foreach($jenis as  $item)
                    @if($value['idj'] == $item['idp'])
                        <td align="right">{!! $value['total'] !!}</td>
                        @if($cc != 1)
                            @if($i > $cc+1)
                                <?php
                                    $si = $simpanan[$key]['idj'];
                                    $ss[$si]['sss'] = $simpanan[$key-$cc]['sld'] +  $value['sld'];
                                ?>
                            @endif
                        @else
                            <?php
                                $si = $simpanan[$key]['idj'];
                                $ss[$si]['sss'] = $simpanan[$key]['sld'];
                            ?>
                        @endif
                    @else
    ...
    <tr>
        <th colspan="3"></th>
        @foreach($jenis as $item)
            <th align="right">{!! number_format($ss[$item['idp']]['sss'], 2, '.', ',') !!}</th>
        @endforeach
