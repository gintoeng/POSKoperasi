...
<table width="100%">
    ...
    <?php $s = 0; $t = 0;?>
    <tr>
        <td colspan="6" style="background: #eaeaea">CASH</td>
    </tr>
    @foreach($transaksi as $value)
        @if($value->type_pembayaran == "cash")
        <?php $s+=$value->jumlah; ?>
        <tr>
    ...
    @endforeach
    <tr>
        <td colspan="6" style="background: #eaeaea">TUNDA</td>
    </tr>
    @foreach($transaksi as $value)
        @if($value->type_pembayaran == "tunda")
        <?php $t+=$value->jumlah; ?>
        <tr>
            <td align="center">{!! $i++ !!}</td>
            <td>{!! $value->noref !!}</td>
    ...
    <tr>
        ...
        <th align="right">{!! number_format($s+$t, 2, '.',',') !!}</th>
        <th colspan="2"></th>
    </tr>
    </tfoot>
</table>
