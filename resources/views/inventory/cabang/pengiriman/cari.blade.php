...
<h1 style="margin-left:4%; margin-top:3%;"><a href="{!! url('/inventory') !!}" class="nav-button transform"><span></span></a>&nbsp;Pengiriman Barang</h1>
<hr style="width:95%;"><br>
<div class="grid">
    <div class="row cells12">
        <div class="cell"></div>
        <div class="cell colspan10">
            <form action="{{url('inventory/cabang/pengiriman/search')}}" method="get">
                <div class="row cells12">
                    ...
                    <div class="cell colspan4">
                        ...
                        <a href="{{url('inventory/cabang/pengiriman')}}"><button type="button" class="button warning"><span class="mif-history"></span></button></a>
                        {{csrf_field()}}
                    </div>
                    <div class="cell colspan4">
                        <a class="place-right" onclick="ceknomor('#dialog')" ><button type="button" class="button success"><span class="mif-paper-plane mif-ani-float"></span>Tambah Data Pengiriman</button></a>
                    </div>
                </div>
            </form>
            ...
            <table class="dataTable striped hovered cell-hovered border bordered " data-searching="true" style="overflow:auto; width:100%; background-color:#fff; width:100%;">
                ...
                <tr>
                    ...
                    <th class="text-center ribbed-cyan fg-white padding10 text-shadow">No. Pengiriman</th>
                ...
                @foreach($header as $headers)
                    @if($headers->id_cabang == \Illuminate\Support\Facades\Auth::user()->cabang && $headers->tipe == "cbkirim")
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$headers->nopembelian}}</td>
                            <td>{{$headers->tanggal}}</td>
                            <td>{{$headers->cabang->nama}}</td>
                            <td>{{$headers->terima->nama}}</td>
                            <td>{{$headers->status}}</td>
                            <td>Rp. {{number_format($headers->detail->SUM('sub_total'), '2')}}</td>
                            <td colspan="3"><center>
                                    <a href="{{url('inventory/cabang/pengiriman/editheader/'.$headers->id)}}"><button type="button" class="button {{$headers->start == "1" ? 'bg-grayLight fg-white' : 'warning'}}" align="center" {{$headers->start == "1" ? 'disabled' : ''}}>Edit</button></a>
                                    <a href="{{url('inventory/cabang/pengiriman/hapusheader/'.$headers->id)}}"><button type="button" class="button danger" align="center">Hapus</button></a>
                                    <a href="{{url('inventory/cabang/pengiriman/detail/'.$headers->id)}}"><button type="button" class="button info" align="center">Detail</button></a></center></td>
                        </tr>
                    @endif
                @endforeach
                </tbody>
...
<script>
    ...
    function ceknomor(id){
        $.ajax({
            url: "{!! url('inventory/cabang/pengiriman/ceknomor') !!}",
            data: {},
            dataType: "json",
            type: "get",
            success:function(data)
            {
                if (data[0]["stat"] == "kosong") {
                    ...
                    location.href = "{!! url('inventory/cabang/pengiriman/tambah') !!}";
                }
            }
