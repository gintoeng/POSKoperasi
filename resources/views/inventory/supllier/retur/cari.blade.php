...
<h1 style="margin-left:4%; margin-top:3%;"><a href="{!! url('/inventory') !!}" class="nav-button transform"><span></span></a>&nbsp;Form Retur Pembelian Barang</h1>
<hr style="width:95%;"><br>
<div class="grid">
    <div class="row cells12">
        <div class="cell"></div>
        <div class="cell colspan10">
            <form action="{{url('inventory/supplier/retur/search')}}" method="get">
                <div class="row cells12">
                    ...
                    <div class="cell colspan4">
                        ...
                        <a href="{{url('inventory/supplier/retur')}}"><button type="button" class="button warning"><span class="mif-history"></span></button></a>
                        {{csrf_field()}}
                    </div>
                    <div class="cell colspan4">
                        <a class="place-right" onclick="ceknomor('#dialog')" ><button type="button" class="button success"><span class="mif-paper-plane mif-ani-float"></span>Tambah Data Retur</button></a>
                    </div>
                </div>
            </form>
            ...
            <table class="dataTable striped hovered cell-hovered border bordered " data-searching="true" style="overflow:auto; width:100%; background-color:#fff; width:100%;">
                ...
                <tr>
                    ...
                    <th class="text-center ribbed-cyan fg-white padding10 text-shadow">No. Retur</th>
                    ...
                    <th class="text-center ribbed-cyan fg-white padding10 text-shadow">Jenis Retur</th>
                ...
                @foreach($header as $headers)
                    @if($headers->id_cabang == \Illuminate\Support\Facades\Auth::user()->cabang && $headers->tipe == "retur")
                    <tr>
                        <td>{{$i++}}</td>
                        ...
                        <td>{{$headers->jenis_retur}}</td>
                        <td>Rp. {{number_format($headers->detail->SUM('sub_total'), '2')}}</td>
                        <td colspan="3"><center><a href="{{url('inventory/supplier/retur/editheader/'.$headers->id)}}" ><button type="button" class="button {{$headers->start == 1 ? 'bg-grayLight fg-white' : 'warning'}}" align="center" {{$headers->start == 1 ? 'disabled' : ''}}>Edit</button></a>
                                <a href="{{url('inventory/supplier/retur/hapusheader/'.$headers->id)}}"class="button danger" align="center">Hapus</a>
                                <a href="{{url('inventory/supplier/retur/detail/'.$headers->id)}}"class="button info" align="center">Detail</a></center></td>
                    </tr>
                    @endif
                @endforeach
                </tbody>
...
<div data-role="dialog" id="dialog" class="padding20" data-close-button="true" data-windows-style="true" data-overlay="true" data-overlay-color="op-dark" data-overlay-click-close="true">
    <div class="container">
        ...
        <p>
            Format nomor untuk retur vendor belum disetting, silahkan setting terlebih dahulu!
...
<script>
    ...
    function ceknomor(id){
        $.ajax({
            url: "{!! url('inventory/supplier/retur/ceknomor') !!}",
            data: {},
            dataType: "json",
            type: "get",
            success:function(data)
            {
                if (data[0]["stat"] == "kosong") {
                    ...
                    location.href = "{!! url('inventory/supplier/retur/tambah') !!}";
                }
            }
