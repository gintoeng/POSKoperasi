...
<h1 style="margin-left:4%; margin-top:3%;"><a href="{!! url('/inventory') !!}" class="nav-button transform"><span></span></a>&nbsp;Penerimaan Supplier</h1>
<hr style="width:95%;"><br>
<div class="grid">
    <div class="row cells12">
        <div class="cell"></div>
        <div class="cell colspan10">
            <form action="{{url('inventory/supplier/penerimaan/search')}}" method="get">
                <div class="row cells12">
                    ...
                    <div class="cell colspan4">
                        ...
                        <a href="{{url('inventory/supplier/penerimaan')}}"><button type="button" class="button warning"><span class="mif-history"></span></button></a>
                        {{csrf_field()}}
                    </div>
                    <div class="cell colspan4">
            ...
            <table class="dataTable striped hovered cell-hovered border bordered " data-searching="true" style="overflow:auto; width:100%; background-color:#fff; width:100%;">
                ...
                @foreach($header as $headers)
                    @if($headers->id_cabang == \Illuminate\Support\Facades\Auth::user()->cabang && $headers->tipe == "penerimaan")
                    <tr>
                        <td>{{$i++}}</td>
                        ...
                        <td colspan="3"><center>
                                ...
                                @else
                                    <a href="javascript:void(0)"><button type="button" class="button bg-grayLight fg-white"  align="center" disabled>Edit</button></a>
                                @endif
                                <a href="{{url('inventory/supplier/penerimaan/hapusheader/'.$headers->id)}}"class="button danger" align="center">Hapus</a>
                                <a href="{{url('inventory/supplier/penerimaan/detail/'.$headers->id)}}"class="button info" align="center">Detail</a>
                                @if($headers->receive == 0)
                                    <a href="{{url('inventory/supplier/penerimaan/receive/'.$headers->id)}}"><button type="button" class="button primary" align="center">Receive</button></a>
                                @else
...
<script>
    ...
    function ceknomor(id){
        $.ajax({
            url: "{!! url('inventory/supplier/penerimaan/ceknomor') !!}",
            data: {},
            dataType: "json",
            type: "get",
            success:function(data)
            {
                if (data[0]["stat"] == "kosong") {
                    ...
                    location.href = "{!! url('inventory/supplier/penerimaan/tambah') !!}";
                }
            }
