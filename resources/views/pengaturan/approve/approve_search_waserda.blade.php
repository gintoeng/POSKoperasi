                ...
                <div class="panel-body">
                    <div class="box-tab">
                        <ul class="nav nav-tabs">
                            ...
                            <li class="active"><a href="#waserda" data-toggle="tab">Waserda</a>
                            </li>
                        </ul>
                        <div class="tab-content">
                            <div class="tab-pane fade active in" id="waserda">
                                <div class="row">
                                    {{--<div class="col-md-12">--}}
                                    <form class="form-inline" role="form" method="get" action="{{ url('pengaturan/approve/waserda/search') }}">
                                        ...
                                        <a href="{!! url('pengaturan/approve/waserda') !!}"><button type="button" class="btn btn-color btn-sm"><i class="ti ti-reload"> Reset</i></button></a>
                                    </form>
                                    {{--</div>--}}
                                </div>
                                        ...
                                        <div class="table-responsive no-border">
                                            <table class="table table-bordered table-striped no-m">
                                                ...
                                                @foreach($approve as $value)
                                                    <?php
                                                    $lev1 = \App\Approverole::where('for', 'waserda')->where('id_for', $value->pembwasid->id_cabang)->where('level', '1')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                    $lev2 = \App\Approverole::where('for', 'waserda')->where('id_for', $value->pembwasid->id_cabang)->where('level', '2')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                    $lev3 = \App\Approverole::where('for', 'waserda')->where('id_for', $value->pembwasid->id_cabang)->where('level', '3')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                    $rel = \App\Approverole::where('for', 'waserda')->where('id_for', $value->pembwasid->id_cabang)->where('level', '4')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{!! $value->pembwasid->nopembelian !!}</td>
                                                        <td>{!! $value->pembwasid->tanggal !!}</td>
                                                        <td>{!! $value->pembwasid->cabang->nama !!}</td>
                                                        <td class="text-right">{!! number_format($value->pembwasid->detail->sum('sub_total'), 2, '.', ',') !!}</td>
