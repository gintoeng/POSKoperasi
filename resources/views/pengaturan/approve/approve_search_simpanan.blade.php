                ...
                <div class="panel-body">
                    <div class="box-tab">
                        <ul class="nav nav-tabs">
                            ...
                            <li><a href="{!! url('pengaturan/approve/pinjaman') !!}">Pinjaman</a>
                            </li>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade active in" id="simpanan">
                                <div class="row">
                                    {{--<div class="col-md-12">--}}
                                    <form class="form-inline" role="form" method="get" action="{{ url('pengaturan/approve/simpanan/search') }}">
                                        <div class="form-group mr5">
                                            <label class="control-label" for="cari">&nbsp;&nbsp;&nbsp;</label>
                                            <input name="query" type="text" class="form-control" value="{{$query}}" placeholder="Cari simpanan" id="cari">
                                        </div>
                                        <input type="hidden" name="search" value="1">
                                        <button type="submit" class="btn btn-sm btn-primary"><i class="ti ti-search"> Cari</i></button>
                                        <a href="{!! url('pengaturan/approve/simpanan') !!}"><button type="button" class="btn btn-color btn-sm"><i class="ti ti-reload"> Reset</i></button></a>
                                    </form>
                                    {{--</div>--}}
                                </div>
                                        ...
                                        <div class="table-responsive no-border">
                                            <table class="table table-bordered table-striped no-m">
                                                ...
                                                @foreach($approve as $value)
                                                    <?php
                                                        $lev1 = \App\Approverole::where('for', 'simpanan')->where('id_for', $value->transimpid->simpananid->jenis_simpanan)->where('level', '1')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                        $lev2 = \App\Approverole::where('for', 'simpanan')->where('id_for', $value->transimpid->simpananid->jenis_simpanan)->where('level', '2')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                        $lev3 = \App\Approverole::where('for', 'simpanan')->where('id_for', $value->transimpid->simpananid->jenis_simpanan)->where('level', '3')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                        $rel = \App\Approverole::where('for', 'simpanan')->where('id_for', $value->transimpid->simpananid->jenis_simpanan)->where('level', '4')->where('id_user', \Illuminate\Support\Facades\Auth::user()->id)->first();
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{!! $value->transimpid->simpananid->nomor_simpanan !!}</td>
                                                        <td>{!! $value->transimpid->simpananid->pengaturanid->jenis_simpanan !!}</td>
                                                        <td>{!! $value->transimpid->simpananid->anggotaid->nama !!}</td>
                                                        <td class="text-right">{!! number_format($value->transimpid->nominal, 2, '.', ',') !!}</td>
