...
{{--                                <a href="{!! url('pinjaman/import') !!}" class="btn btn-success mb15"><i class="ti ti-import"></i> Import</a>--}}
                            ...
                            <form class="form-inline" role="form" method="get" action="{{ url('pinjaman/search') }}">
                                ...
                                <a href="{!! url('pinjaman') !!}"><button type="button" class="btn btn-color btn-sm"><i class="ti ti-reload"> Reset</i></button></a>
                            </form>
                        </div>
                        <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <div class="radio" id="realis">
                                            <label><input id="all" name="realisasi" type="radio" value="all" {!! $radio == "all" ? 'checked' : '' !!}>ALL</label>&nbsp;&nbsp;&nbsp;&nbsp;<label><input id="real" name="realisasi" type="radio" value="belum" {!! $radio == "belum" ? 'checked' : '' !!}>Belum Realisasi</label>&nbsp;&nbsp;&nbsp;&nbsp;
                                            <label><input id="real2" name="realisasi" type="radio" value="sudah" {!! $radio == "sudah" ? 'checked' : '' !!}>Sudah Realisasi</label>&nbsp;&nbsp;&nbsp;&nbsp;
                                            <label><input id="real3" name="realisasi" type="radio" value="lunas" {!! $radio == "lunas" ? 'checked' : '' !!}>Sudah Lunas</label>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-9">
                                        <select name="kolektibilitas" type="text" class="form-control chosen" id="kolektibilitas" data-placeholder="Pilih Kolektibilitas">
                                            <option value="0" {!! $pilih == "0" ? 'selected' : '' !!}>ALL</option>
                                            @foreach($kolektibilitas as $value)
                                                <option value="{!! $value->id !!}" {!! $pilih == $value->id ? 'selected' : '' !!}>{!! $value->kode !!} - {!! $value->keterangan !!}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
