...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <input type="hidden" value="{{$pinj->id}}" id="nopinj" name="nomor_pinjaman">
                                    <label for="nomor_pinjaman" class="col-sm-3 control-label">Nomor Pinjaman</label>
                                    <div class="col-sm-9">
                                        <select name="nopinj" style="width: 100%;" type="text" class="form-controlm" id="nomor_pinjaman" data-placeholder="Pilih Nomor Pinjaman" disabled>
                                            <option value=""></option>
                                            @foreach($pinjaman as $value)
                                                <option value="{!! $value->id !!}" {{$pinj->id == $value->id ? 'selected' : ''}}>{!! $value->nomor_pinjaman !!} - {!! $value->anggotaid->nama !!} ( {!! $value->kolektibilitasid->kode !!} )</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
    ...
    <script>
        ...
        $("#nomor_pinjaman").removeAttr('class');
        $("#nomor_pinjaman").select2();
...
//                $('#jmlpengajuan_simulasi').val($('#jumlah_pengajuan').val());
        ...
        //$('#nomor_pinjaman').on('change', function () {
            $.ajax({
                url: "{!! url('pinjaman/realisasi/ajax/') !!}/" + $('#nopinj').val(),
                data: {},
                dataType: "json",
                type: "get",
                success:function(data)
            ...
            $('#tjamin').load("{!! url('pinjaman/realisasi/tjamin') !!}/" + $('#nopinj').val());
